...
const SUGGESTIONS = ['Abre la Golden','Buscar Cascade','Temperatura actual','Mostrar documentos'];
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1">
            <input value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')send();}}
              ...
              style={{background:'rgba(13,24,36,0.8)',border:'1px solid rgba(255,255,255,0.07)',color:'#e8f0f8'}}
            ...
            <button onClick={send} disabled={!draft.trim()||isActive}
              className="absolute right-2 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl disabled:opacity-30"
              ...
              <Send size={13} style={{color:'#00e1ff'}}/>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <motion.div initial={{opacity:0,y:10,scale:0.97}} animate={{opacity:1,y:0,scale:1}} transition={{duration:0.32,delay,ease:[0.22,1,0.36,1]}}
      ...
      {!user&&<div className="mr-2.5 mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-display text-[9px] font-bold"
        style={{background:'rgba(0,135,255,0.1)',border:'1px solid rgba(0,225,255,0.18)',color:'#00e1ff',boxShadow:'0 0 10px rgba(0,135,255,0.18)'}}>AI</div>}
      <div className="max-w-[76%]">
        <div className="rounded-2xl px-4 py-3"
          style={user?{background:'rgba(255,170,0,0.09)',border:'1px solid rgba(255,170,0,0.18)',borderBottomRightRadius:6}
          ...
          {msg.navigateTo&&!user&&(
            <motion.button initial={{opacity:0,y:5}} animate={{opacity:1,y:0}} transition={{delay:0.28}}
              ...
              className="mt-3 flex items-center gap-1.5 rounded-xl px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider"
              style={{background:'rgba(0,225,255,0.06)',border:'1px solid rgba(0,225,255,0.18)',color:'#00e1ff'}}>
              <ChevronRight size={10}/> Navegar
            </motion.button>
          )}
        </div>
  ...
  return (
    <motion.div initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="flex items-center gap-2.5">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-display text-[9px] font-bold"
        style={{background:'rgba(0,135,255,0.1)',border:'1px solid rgba(0,225,255,0.18)',color:'#00e1ff'}}>AI</div>
