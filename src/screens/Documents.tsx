    ...
    >
      ...
      <div className="mt-3 flex items-center justify-between border-t pt-2.5" style={{borderColor:'rgba(255,255,255,0.05)'}}>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-[10px]" style={{color:'rgba(0,225,255,0.38)'}}>Ver →</span>
        </div>
      </div>
    </motion.button>
  );
...
function DocDetail({doc,onClose}:{doc:DocItem;onClose:()=>void}) {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      style={{background:'rgba(2,4,8,0.85)',backdropFilter:'blur(10px)'}} onClick={onClose}>
      <motion.div initial={{y:55,opacity:0,scale:0.97}} animate={{y:0,opacity:1,scale:1}} exit={{y:40,opacity:0}}
        transition={{duration:0.35,ease:[0.22,1,0.36,1]}} className="hud w-full max-w-lg p-6" onClick={e=>e.stopPropagation()}>
