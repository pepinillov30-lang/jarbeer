...
import { systemStatus, quickActions, type Screen } from '../data/mockData';
...
export function Home({ micState, onMic, onNavigate, soundEnabled, onToggleSound }: HomeProps) {
  ...
  useEffect(()=>{ tmr.current=setInterval(()=>setTime(fmtTime()),1000); return()=>{ if(tmr.current)clearInterval(tmr.current); }; },[]);
    ...
    <div className="flex min-h-full flex-col px-4 pb-32 pt-[max(env(safe-area-inset-top),16px)]">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded px-1.5 py-0.5 font-mono text-[8px] tracking-widest uppercase"
              style={{background:'rgba(255,170,0,0.1)',border:'1px solid rgba(255,170,0,0.22)',color:'#FFAA00'}}>{APP_VERSION}</span>
          </div>
          <p className="font-mono text-[10px] capitalize" style={{color:'rgba(74,96,112,0.7)'}}>{dateStr}</p>
        </div>
        <div className="flex items-center gap-2">
          <motion.button whileTap={{scale:0.92}} onClick={onToggleSound}
      ...
      <motion.div initial={{opacity:0,scale:0.88}} animate={{opacity:1,scale:1}}
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.3}}
          ...
          style={{textShadow:'0 0 40px rgba(255,255,255,0.12)'}}>{time}</motion.div>
        <Avatar size={280} active={isActive} responding={isResponding}/>
        <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.55}} className="mt-4 text-center">
      ...
      <GlassCard className="mt-3 flex items-center gap-3 px-4 py-3" delay={0.42}>
        ...
        <span className="flex-1 font-mono text-xs" style={{color:'rgba(74,96,112,0.8)'}}>Red · Modo producción</span>
        <span className="flex items-center gap-1.5"><span className="live-dot"/><span className="font-mono text-xs" style={{color:'#34d399'}}>{systemStatus.network}</span></span>
      </GlassCard>

      {/* Mic */}
      <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:0.46}} className="mt-7 flex flex-col items-center">
          ...
          {quickActions.map((action,idx)=>{
            ...
            return (
              ...
              >
                ...
                {action.value && <span className="shrink-0 rounded-lg px-2.5 py-1 font-mono text-xs font-bold"
                  style={{background:'rgba(255,170,0,0.09)',border:'1px solid rgba(255,170,0,0.18)',color:'#FFAA00'}}>{action.value}</span>}
                <ArrowRight size={15} className="shrink-0" style={{color:'rgba(74,96,112,0.45)'}}/>
              </motion.button>
            );
          })}
        </div>
...
function MetricCard({icon,label,value,sub,accent,delay}:{icon:React.ReactNode;label:string;value:string;sub:string;accent:string;delay:number}) {
  ...
  return (
    <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.5,delay,ease:[0.22,1,0.36,1]}}
      className="rounded-2xl p-4" style={{background:c.bg,border:`1px solid ${c.border}`,boxShadow:`0 4px 18px ${c.glow}`}}>
