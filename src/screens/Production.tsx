...
export function Production() {
  ...
  return (
    <div className="flex min-h-full flex-col pb-32">
      <ScreenHeader title="Ficha de Producción" subtitle={`Lote ${fields.batch} · ${productionData.recipe}`}
        right={<span className="rounded-lg px-3 py-1 font-mono text-[10px] uppercase tracking-wider"
          style={{background:'rgba(0,225,255,0.07)',border:'1px solid rgba(0,225,255,0.18)',color:'#00e1ff'}}>{productionData.stage}</span>}/>
      <div className="flex flex-col gap-4 px-4">
        ...
        <GlassCard className="overflow-hidden px-0 py-0" corners delay={0.08}>
          <div className="flex overflow-x-auto px-4 pb-3 pt-4" style={{scrollbarWidth:'none'}}>
            {productionData.timeline.map((item,i)=>{
              const isCur=i===stageIdx, isDone=item.done;
              return (
                <div key={item.stage} className="flex shrink-0 flex-col items-center" style={{minWidth:70}}>
                  <div className="relative flex flex-col items-center">
                    <div className="relative flex h-9 w-9 items-center justify-center rounded-full"
                      ...
                      {isCur&&<motion.div className="absolute inset-0 rounded-full" style={{border:'1px solid rgba(255,170,0,0.38)'}}
                        animate={{scale:[1,1.65],opacity:[0.55,0]}} transition={{duration:2,repeat:Infinity,ease:'easeOut'}}/>}
                    </div>
                    {i<productionData.timeline.length-1&&<div className="absolute left-[calc(50%+18px)] top-4 h-px" style={{width:34,background:isDone?'rgba(0,225,255,0.28)':'rgba(255,255,255,0.06)'}}/>}
                  </div>
                  <span className="mt-2 text-center font-mono text-[8px] leading-tight"
                    style={{color:isDone?'#00e1ff':isCur?'#FFAA00':'rgba(74,96,112,0.45)',maxWidth:60}}>{item.stage}</span>
                </div>
              );
            })}
          </div>
        ...
        <Sec title="Datos generales" icon={<FileText size={14}/>} delay={0.12}>
          <div className="grid grid-cols-2 gap-3">
            {([
              ...
              {key:'currentTemp',label:'Temp. (°C)',icon:<Thermometer size={12}/>},
        ...
        <Sec title="Métricas técnicas" icon={<Beaker size={14}/>} delay={0.18}>
          <div className="grid grid-cols-3 gap-3">
            {[{l:'OG',v:productionData.og.toFixed(3)},{l:'FG',v:productionData.fg.toFixed(3)},{l:'ABV',v:`${productionData.abv}%`},
              {l:'IBU',v:String(productionData.ibu)},{l:'EBC',v:String(productionData.ebc)},{l:'CO₂',v:`${productionData.carbonation}`}].map(m=>(
        ...
        {/* Maltas */}
        <Sec title="Maltas" icon={<Leaf size={14}/>} delay={0.22}
          action={<AddBtn onClick={()=>setMaltas(p=>[...p,{id:`m${Date.now()}`,name:'Nueva malta',amount:'0 kg',ebc:0,supplier:'—'}])}/>}>
          <div className="space-y-2">{maltas.map(m=><IRow key={m.id} c1={m.name} c2={m.amount} c3={`EBC ${m.ebc}`} badge={m.supplier} accent="cyan"/>)}</div>
        </Sec>

        {/* Lúpulos */}
        <Sec title="Lúpulos" icon={<FlaskConical size={14}/>} delay={0.26}
          action={<AddBtn onClick={()=>setLupulos(p=>[...p,{id:`l${Date.now()}`,name:'Nuevo lúpulo',alpha:'0%',amount:'0 g',addition:'—'}])}/>}>
          <div className="space-y-2">{lupulos.map(l=><IRow key={l.id} c1={l.name} c2={l.amount} c3={l.alpha} badge={l.addition} accent="amber"/>)}</div>
        </Sec>
          ...
          {editing==='observations'
            ? <div className="space-y-2">
                <textarea value={fields.observations} onChange={e=>setFields(p=>({...p,observations:e.target.value}))}
                  className="w-full resize-none rounded-xl p-3 font-sans text-sm focus:outline-none"
                  style={{minHeight:80,background:'rgba(13,24,36,0.7)',border:'1px solid rgba(0,225,255,0.3)',color:'#e8f0f8'}} autoFocus/>
            ...
            : <button onClick={()=>setEditing('observations')} className="w-full rounded-xl p-3 text-left"
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.4}} className="pb-4 pt-2">
          <button onClick={handlePdf} disabled={pdfState!=='idle'}
            className="relative w-full overflow-hidden rounded-2xl py-4 font-display text-sm font-bold uppercase tracking-widest"
...
function AddBtn({onClick}:{onClick:()=>void}) {
  return (
    <button onClick={onClick} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider"
      style={{background:'rgba(0,225,255,0.06)',border:'1px solid rgba(0,225,255,0.16)',color:'#00e1ff'}}><Plus size={11}/> Añadir</button>
  );
}
...
function EditField({label,icon,value,isEditing,onEdit,onSave,onCancel}:{fkey:string;label:string;icon:React.ReactNode;value:string;isEditing:boolean;onEdit:()=>void;onSave:(v:string)=>void;onCancel:()=>void;}) {
  ...
  return (
    <button onClick={onEdit} className="rounded-xl p-3 text-left transition-all duration-200"
