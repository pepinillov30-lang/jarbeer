      ...
      <div className="flex flex-col gap-4 px-4">
                ...
                <div key={item.stage} className="flex shrink-0 flex-col items-center" style={{ minWidth:70 }}>
                  <div className="relative flex flex-col items-center">
                    <div className="relative flex h-9 w-9 items-center justify-center rounded-full transition-all duration-300"
                      style={{ background:isDone?'rgba(0,225,255,0.1)':isCur?'rgba(255,170,0,0.12)':'rgba(13,24,36,0.6)',
                        border:isDone?'1.5px solid rgba(0,225,255,0.35)':isCur?'1.5px solid rgba(255,170,0,0.55)':'1px solid rgba(255,255,255,0.07)' }}>
        ...
        <Sec title="Observaciones" icon={<Pencil size={14}/>} delay={0.34}>
          {editing==='observations'
            ...
            : <button onClick={()=>setEditing('observations')} className="w-full rounded-xl p-3 text-left transition-all duration-200"
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.4}} className="pb-4 pt-2">
          <button onClick={handlePdf} disabled={pdfState!=='idle'}
            ...
            style={{ background:pdfState==='done'?'rgba(52,211,153,0.1)':'rgba(0,225,255,0.07)',
              border:pdfState==='done'?'1px solid rgba(52,211,153,0.3)':'1px solid rgba(0,225,255,0.18)',
...
function EditField({label,icon,value,isEditing,onEdit,onSave,onCancel}:{
