  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden>
      ...
      <motion.div className="absolute rounded-full"
        ...
        animate={{ x:[0,18,0], y:[0,-12,0], opacity:[0.5,1,0.5] }} transition={{ duration:20, repeat:Infinity, ease:'easeInOut' }} />
      <motion.div className="absolute rounded-full"
        style={{ width:240, height:240, bottom:'18%', right:'6%', background:'radial-gradient(circle, rgba(255,170,0,0.025) 0%, transparent 70%)' }}
        animate={{ x:[0,-12,0], y:[0,10,0], opacity:[0.4,0.8,0.4] }} transition={{ duration:25, repeat:Infinity, ease:'easeInOut', delay:4 }} />
      ...
      <motion.div className="absolute left-0 right-0 h-px"
        ...
        animate={{ top:['0%','100%'] }} transition={{ duration:9, repeat:Infinity, ease:'linear', repeatDelay:7 }} />
    </div>
  );
}
