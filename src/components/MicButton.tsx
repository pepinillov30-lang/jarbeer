  ...
  return (
    <div className="flex flex-col items-center gap-2.5">
      <div className="relative flex items-center justify-center">
        ...
        <motion.button onClick={onPress} disabled={isActive} whileTap={{ scale:0.92 }}
          animate={{ scale: isListen ? [1,1.05,1] : 1 }}
          transition={isListen ? { duration:0.75, repeat:Infinity } : { duration:0.14 }}
          style={{ width:btnSize, height:btnSize, background:bg, border:`1px solid ${border}`, borderRadius:'50%',
            display:'flex', alignItems:'center', justifyContent:'center', cursor:isActive?'default':'pointer',
            boxShadow:isActive?`0 0 28px ${isRespond?'rgba(255,170,0,0.28)':'rgba(0,225,255,0.22)'}`:undefined,
            transition:'background 0.3s,border-color 0.3s,box-shadow 0.3s' }}
          aria-label={LABELS[state]}
        >
          {state === 'processing'
            ? <motion.div className="h-5 w-5 rounded-full border-2" style={{ borderColor:'#00e1ff', borderTopColor:'transparent' }}
      ...
      <AnimatePresence mode="wait">
        <motion.span key={state} initial={{ opacity:0, y:4 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-4 }}
          transition={{ duration:0.22 }} className="font-mono text-[10px] uppercase tracking-widest"
