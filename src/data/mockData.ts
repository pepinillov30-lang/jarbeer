...
export const systemStatus = {
  state:'Operativo', uptime:'14d 06:42', activeBatch:'Golden Ale 26-017',
  temperature:64.5, targetTemp:66.0, nextTask:'Medir °Plato',
  network:'Conectado', alerts:0, docsIndexed:6, aiModel:'JAR-1.0', aiStatus:'Activo',
};
...
export const productionData = {
  ...
  currentTemp:64.5, targetTemp:66.0, plato:12.4, ph:5.2,
...
export const initialChat: ChatMessage[] = [
  { id:'m1', role:'assistant', timestamp:'09:41', content:'Sistema en línea. Soy J.A.R.B.E.E.R., tu asistente de producción. Puedo abrir lotes, consultar recetas, registrar métricas y guiarte en cada etapa. ¿Cómo puedo ayudarte?' },
];

export interface VoiceCommand {
  triggers:string[]; userText:string; response:string; action?:'navigate'; target?:Screen;
}
export const voiceCommands: VoiceCommand[] = [
  { triggers:['golden','producción','lote','abre'], userText:'JarBeer, abre la Golden.', response:'Abriendo ficha de producción. Lote 26-017 — Golden Ale — activo, fermentación al 68%.', action:'navigate', target:'production' },
  ...
  { triggers:['documentos','biblioteca'], userText:'Muéstrame los documentos.', response:'Abriendo biblioteca documental. 6 documentos disponibles.', action:'navigate', target:'documents' },
  { triggers:['cascade','lúpulo'],      userText:'Buscar Cascade.', response:'Localizando COA Cascade 2026. Referencia COA-CSC-2026.', action:'navigate', target:'documents' },
...
export const quickActions: { id:Screen; label:string; description:string; icon:string; value?:string }[] = [
